...
void Backend::ComputeBiasAcc(Estimator* estimator){
    int N = 10;
    int M = 3;
    /* Qi.inverse() * (0.5 * G * sum_dt * sum_dt + 
    Pj - Pi - Vi * sum_dt) - corrected_delta_p */

    for(int i = 0; i < N ; i++){
    Eigen::Vector3d delta_v_tmp = estimator->pre_integrations[i]->delta_v;
    Eigen::Vector3d delta_p_tmp = estimator->pre_integrations[i]->delta_p;


    double delta_t_tmp = estimator->pre_integrations[i]->sum_dt;
    Quaterniond q_i_tmp{estimator->Rs[i]};
    Eigen::Vector3d V_i_tmp = estimator->Vs[i];
    Eigen::Vector3d V_i_1_tmp = estimator->Vs[i+1];
    Eigen::Vector3d P_i_tmp = estimator->Ps[i];
    Eigen::Vector3d P_i_1_tmp = estimator->Ps[i+1];

    Eigen::Matrix3d dv_dba_tmp = estimator->pre_integrations[i]->jacobian.block<3, 3>(6, 9);
    Eigen::Matrix3d dp_dba_tmp = estimator->pre_integrations[i]->jacobian.block<3, 3>(0, 9);
    //case v
    Eigen::Matrix3d A = dv_dba_tmp;
    Eigen::Vector3d b = q_i_tmp.inverse()*(G*delta_t_tmp+V_i_1_tmp-V_i_tmp)-delta_v_tmp; 
    //case p
    /* Eigen::Matrix3d A = dp_dba_tmp;
    Eigen::Vector3d b = q_i_tmp.inverse()*(0.5*G*delta_t_tmp*delta_t_tmp
    +P_i_1_tmp-P_i_tmp - V_i_tmp * delta_t_tmp)-delta_p_tmp;  */

    /* tmp_A.block<3, 3>(3*i,0) = A ;
    tmp_b.block<3, 1>(3*i,0) = b ;  */
    
    estimator->A[acc_bias_count*10+i] = A;
    estimator->b[acc_bias_count*10+i] = b;
    }    
    acc_bias_count++;
    if(acc_bias_count==5){
        Eigen::MatrixXd tmp_A(5*3*10,3); 
        Eigen::VectorXd tmp_b(5*3*10,1);
        for(int i=0;i<50;i++){
            tmp_A.block<3, 3>(3*i,0) = estimator->A[i];
            tmp_b.block<3, 1>(3*i,0) = estimator->b[i]; 
            
        }
         Eigen::JacobiSVD<MatrixXd> svd(tmp_A, ComputeThinU | ComputeThinV);
        Eigen::Matrix3d W ;
        W<<svd.singularValues().x(),0,0,
            0,svd.singularValues().y(),0,
            0,0,svd.singularValues().z();
        Eigen::Vector3d x = svd.matrixV()*W.inverse()*svd.matrixU().transpose()*tmp_b;
        cout<<"----------------BIAS_ACC------------------------------"<<endl;
        ROS_INFO("compute Bias_Acc is %f , %f, %f !!!",x.x(),x.y(),x.z());
        ROS_INFO("compute Bias_gyro is %f , %f, %f !!!",estimator->Bgs[9].x(),estimator->Bgs[9].y(),estimator->Bgs[9].z());
        ROS_INFO("compute Bias_Acc is %f , %f, %f !!!",estimator->Bas[9].x(),estimator->Bas[9].y(),estimator->Bas[9].z());
         
        acc_bias_count=0;

        //save data
        std::ofstream f;
        f.open("/home/e/catkin_ws_VI_SLAM_estimater_2/src/bias_acc.txt",ios_base::app);
        f << x.x() << " " << x.y() << " " << x.z() << " ";
        f << endl;
        f.close();

        f.open("/home/e/catkin_ws_VI_SLAM_estimater_2/src/bias_gyro.txt",ios_base::app);
        f << estimator->Bgs[9].x() << " " << estimator->Bgs[9].y() << " " << estimator->Bgs[9].z() << " ";
        f << endl;
        f.close();
    }
    }
