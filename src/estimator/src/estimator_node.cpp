...
//用std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr>>来存放数据

//把图片和IMU数据根据时间对齐
/*让一次measure的结果是两个图片内的IMU，和一张图片
 ----------------------img  
 |---|----|---|--|--|--|--|
...
std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr>> getMeasurements()
{
    ...
    {   
        ...
        if (imu_buf.empty() || feature_buf.empty())
            return measurements;
        //estimator.td可以用于补偿已知的时间误差
        //IMU最新的时间戳imu_buf.back() 小于 最旧的图像帧feature_buf.front()，说明IMU数据太少，等待IMU刷新
        ...
        /* IMU最旧的时间戳imu_buf.front()  大于 图像帧最旧的时间戳feature_buf.front()，
        最开始只有图像，过了很久来了IMU，但是第一帧图像太旧了，要丢弃第一帧图像 */
...
void imu_callback(const sensor_msgs::ImuConstPtr &imu_msg)
{
    ...
    m_buf.lock();
    imu_buf.push(imu_msg);//接收fIMU的数据,放到imu_buf中去
    m_buf.unlock();
    con.notify_one();
...
void feature_callback(const sensor_msgs::PointCloudConstPtr &feature_msg)
{
    ...
    m_buf.lock();
    feature_buf.push(feature_msg);//接收feature_tracking跟踪来的数据,放到feature_buf中去
    m_buf.unlock();
    con.notify_one();
}
                ...
                sensor_msgs::PointCloudConstPtr &img_msg)
{
    ...
    /* 大部分的t都是在上一个img_t和这一个img_t之间，仅在最后一个数据有时间超出的情况
        所以else应该只执行一次*/
    if (t <= img_t)
    { 
        if (current_time < 0)
            current_time = t;
        double dt = t - current_time;//dt是两个imu的采样间隔
        ROS_ASSERT(dt >= 0);
        current_time = t;
    ...
    else//在euroc中不会出现这样的情况，这种情况更多的还是为了实际应用场景来考虑
    {
        ROS_INFO("-----t <= img_t-----");
...
void processVIO(sensor_msgs::PointCloudConstPtr& img_msg)
{
    ...
    {
        int v = img_msg->channels[0].values[i] + 0.5;//四舍五入
        int feature_id = v / 1;
        int camera_id = v % 1;
...
void visualize(sensor_msgs::PointCloudConstPtr &relo_msg,std_msgs::Header &header)
{
    ...
    if(SHOW_DETAIL_INFO)
        ROS_INFO("Estimated Data pub into topic... \n");
    if (relo_msg != NULL)
...
{   
    ...
    for (auto &measurement : measurements)
    {
        ...
        if(SHOW_DETAIL_INFO){
            /* 用来检验每次的imu和img的时间轴 */
        ROS_INFO("img time is %f ",img_msg->header.stamp.toSec());
        auto imu_msg_tmp = measurement.first;
        int imu_msg_tmp_size = imu_msg_tmp.size();
        ROS_INFO("imu data size is %d ",imu_msg_tmp_size);
        auto imu_msg_tmp_start = imu_msg_tmp.front();
        auto imu_msg_tmp_end =imu_msg_tmp.back();
        ROS_INFO("imu start time is %f ",imu_msg_tmp_start->header.stamp.toSec());
        ROS_INFO("imu end time is %f \n",imu_msg_tmp_end->header.stamp.toSec());
        }
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle n;
    ROS_INFO("\033[1;32m----> estimator Started.\033[0m");
    ...
    ros::Subscriber sub_image = n.subscribe("feature", 2000, feature_callback);//接收特征数据，包含编号，像素点，移动速度
